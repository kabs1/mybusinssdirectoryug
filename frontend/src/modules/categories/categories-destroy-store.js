  ...
  actions: {
    async doDestroy({ commit, dispatch, rootGetters }, id) {
      try {
        ...
        Message.success(i18n('entities.categories.destroy.success'));

        routerAsync().push('/categories');

        dispatch(
          `${'categories/list'}/doFetch`,
          rootGetters[`${'categories/list'}/filter`],
          {
            root: true,
          },
        );
    ...
    async doDestroyAll(
      ...
      try {
        ...
        dispatch(`${'categories/list'}/doUnselectAll`, null, {
          root: true,
        });
