...
import damagesListExporterFields from '@/modules/damages/damages-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!damagesListExporterFields || !damagesListExporterFields.length) {
          throw new Error('damagesListExporterFields is required');
        }
        ...
        const response = await DamagesService.list(
          filter,
          getters.orderBy,
          null,
          null,
