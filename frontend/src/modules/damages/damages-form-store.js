  ...
  actions: {
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await DamagesService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(i18n('entities.damages.create.success'));
        routerAsync().push('/damages');
      } catch (error) {
        Errors.handle(error);
        commit('CREATE_ERROR');
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await DamagesService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(i18n('entities.damages.update.success'));
        routerAsync().push('/damages');
      } catch (error) {
        Errors.handle(error);
        commit('UPDATE_ERROR');
