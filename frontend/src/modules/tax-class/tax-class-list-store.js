...
import taxClassListExporterFields from '@/modules/tax-class/tax-class-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!taxClassListExporterFields || !taxClassListExporterFields.length) {
          throw new Error('taxClassListExporterFields is required');
        }
        ...
        const response = await TaxClassService.list(
          filter,
          getters.orderBy,
          null,
          null,
