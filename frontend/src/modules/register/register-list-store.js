...
import registerListExporterFields from '@/modules/register/register-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!registerListExporterFields || !registerListExporterFields.length) {
          throw new Error('registerListExporterFields is required');
        }
        ...
        const response = await RegisterService.list(
          filter,
          getters.orderBy,
          null,
          null,
