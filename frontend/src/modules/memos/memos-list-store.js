...
import memosListExporterFields from '@/modules/memos/memos-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!memosListExporterFields || !memosListExporterFields.length) {
          throw new Error('memosListExporterFields is required');
        }
