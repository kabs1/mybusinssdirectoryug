...
import leaveListExporterFields from '@/modules/leave/leave-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!leaveListExporterFields || !leaveListExporterFields.length) {
          throw new Error('leaveListExporterFields is required');
        }
