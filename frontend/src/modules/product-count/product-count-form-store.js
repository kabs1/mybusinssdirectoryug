  ...
  actions: {
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await ProductCountService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(i18n('entities.productCount.create.success'));
        routerAsync().push('/product-count');
      } catch (error) {
        Errors.handle(error);
        commit('CREATE_ERROR');
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await ProductCountService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(i18n('entities.productCount.update.success'));
        routerAsync().push('/product-count');
      } catch (error) {
        Errors.handle(error);
        commit('UPDATE_ERROR');
