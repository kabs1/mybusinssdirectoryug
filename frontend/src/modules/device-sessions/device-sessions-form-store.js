  ...
  actions: {
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await DeviceSessionsService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(i18n('entities.deviceSessions.create.success'));
        routerAsync().push('/device-sessions');
      } catch (error) {
        Errors.handle(error);
        commit('CREATE_ERROR');
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await DeviceSessionsService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(i18n('entities.deviceSessions.update.success'));
        routerAsync().push('/device-sessions');
      } catch (error) {
        Errors.handle(error);
        commit('UPDATE_ERROR');
