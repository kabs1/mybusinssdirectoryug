  ...
  actions: {
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await AllowanceAndDeductionService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(i18n('entities.allowanceAndDeduction.create.success'));
        routerAsync().push('/allowance-and-deduction');
      } catch (error) {
        Errors.handle(error);
        commit('CREATE_ERROR');
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await AllowanceAndDeductionService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(i18n('entities.allowanceAndDeduction.update.success'));
        routerAsync().push('/allowance-and-deduction');
      } catch (error) {
        Errors.handle(error);
        commit('UPDATE_ERROR');
