...
import priceChangeListExporterFields from '@/modules/price-change/price-change-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!priceChangeListExporterFields || !priceChangeListExporterFields.length) {
          throw new Error('priceChangeListExporterFields is required');
        }
        ...
        const response = await PriceChangeService.list(
          filter,
          getters.orderBy,
          null,
          null,
