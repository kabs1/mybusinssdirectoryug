...
import variationListExporterFields from '@/modules/variation/variation-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!variationListExporterFields || !variationListExporterFields.length) {
          throw new Error('variationListExporterFields is required');
        }
        ...
        const response = await VariationService.list(
          filter,
          getters.orderBy,
          null,
          null,
