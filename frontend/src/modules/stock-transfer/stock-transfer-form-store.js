  ...
  actions: {
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        Message.success(i18n('entities.stockTransfer.create.success'));
        routerAsync().push('/stock-transfer');
      } catch (error) {
        Errors.handle(error);
        commit('CREATE_ERROR');
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await StockTransferService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(i18n('entities.stockTransfer.update.success'));
        routerAsync().push('/stock-transfer');
      } catch (error) {
        Errors.handle(error);
        commit('UPDATE_ERROR');
