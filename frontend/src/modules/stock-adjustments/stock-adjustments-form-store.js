  ...
  actions: {
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await StockAdjustmentsService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(i18n('entities.stockAdjustments.create.success'));
        routerAsync().push('/stock-adjustments');
      } catch (error) {
        Errors.handle(error);
        commit('CREATE_ERROR');
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await StockAdjustmentsService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(i18n('entities.stockAdjustments.update.success'));
        routerAsync().push('/stock-adjustments');
      } catch (error) {
        Errors.handle(error);
        commit('UPDATE_ERROR');
