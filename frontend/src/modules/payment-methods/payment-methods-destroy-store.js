  ...
  actions: {
    async doDestroy({ commit, dispatch, rootGetters }, id) {
      try {
        ...
        Message.success(i18n('entities.paymentMethods.destroy.success'));

        routerAsync().push('/payment-methods');

        dispatch(
          `${'paymentMethods/list'}/doFetch`,
          rootGetters[`${'paymentMethods/list'}/filter`],
          {
            root: true,
          },
        );
    ...
    async doDestroyAll(
      ...
      try {
        ...
        dispatch(`${'paymentMethods/list'}/doUnselectAll`, null, {
          root: true,
        });
