...
import holidayListExporterFields from '@/modules/holiday/holiday-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!holidayListExporterFields || !holidayListExporterFields.length) {
          throw new Error('holidayListExporterFields is required');
        }
