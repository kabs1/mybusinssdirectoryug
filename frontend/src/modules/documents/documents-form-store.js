  ...
  actions: {
    ...
    async doFind({ commit }, id) {
      try {
        ...
        const record = await DocumentsService.find(id);
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        Message.success(i18n('entities.documents.create.success'));
        routerAsync().push('/documents');
      } catch (error) {
        Errors.handle(error);
        commit('CREATE_ERROR');
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        Message.success(i18n('entities.documents.update.success'));
        routerAsync().push('/documents');
      } catch (error) {
        Errors.handle(error);
        commit('UPDATE_ERROR');
