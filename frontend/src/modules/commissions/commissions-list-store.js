...
import commissionsListExporterFields from '@/modules/commissions/commissions-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!commissionsListExporterFields || !commissionsListExporterFields.length) {
          throw new Error('commissionsListExporterFields is required');
        }
        ...
        const response = await CommissionsService.list(
          filter,
          getters.orderBy,
          null,
          null,
