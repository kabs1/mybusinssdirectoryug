...
import staffListExporterFields from '@/modules/staff/staff-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!staffListExporterFields || !staffListExporterFields.length) {
          throw new Error('staffListExporterFields is required');
        }
