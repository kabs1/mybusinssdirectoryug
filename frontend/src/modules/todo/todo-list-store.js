...
import todoListExporterFields from '@/modules/todo/todo-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!todoListExporterFields || !todoListExporterFields.length) {
          throw new Error('todoListExporterFields is required');
        }
