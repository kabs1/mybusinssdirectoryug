  ...
  actions: {
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await LeaveTypeService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(i18n('entities.leaveType.create.success'));
        routerAsync().push('/leave-type');
      } catch (error) {
        Errors.handle(error);
        commit('CREATE_ERROR');
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await LeaveTypeService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(i18n('entities.leaveType.update.success'));
        routerAsync().push('/leave-type');
      } catch (error) {
        Errors.handle(error);
        commit('UPDATE_ERROR');
