  ...
  actions: {
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await DocumentTypesService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(i18n('entities.documentTypes.create.success'));
        routerAsync().push('/document-types');
      } catch (error) {
        Errors.handle(error);
        commit('CREATE_ERROR');
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await DocumentTypesService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(i18n('entities.documentTypes.update.success'));
        routerAsync().push('/document-types');
      } catch (error) {
        Errors.handle(error);
        commit('UPDATE_ERROR');
