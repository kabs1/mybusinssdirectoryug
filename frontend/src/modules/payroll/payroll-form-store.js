  ...
  actions: {
    ...
    async doCreate({ commit }, values) {
      try {
        ...
        await PayrollService.create(values);
        commit('CREATE_SUCCESS');
        Message.success(i18n('entities.payroll.create.success'));
        routerAsync().push('/payroll');
      } catch (error) {
        Errors.handle(error);
        commit('CREATE_ERROR');
    ...
    async doUpdate({ commit }, { id, values }) {
      try {
        ...
        await PayrollService.update(id, values);

        commit('UPDATE_SUCCESS');
        Message.success(i18n('entities.payroll.update.success'));
        routerAsync().push('/payroll');
      } catch (error) {
        Errors.handle(error);
        commit('UPDATE_ERROR');
