...
import productLogsListExporterFields from '@/modules/product-logs/product-logs-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!productLogsListExporterFields || !productLogsListExporterFields.length) {
          throw new Error('productLogsListExporterFields is required');
        }
        ...
        const response = await ProductLogsService.list(
          filter,
          getters.orderBy,
          null,
          null,
