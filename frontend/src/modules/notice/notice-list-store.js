...
import noticeListExporterFields from '@/modules/notice/notice-list-exporter-fields';
...
export default {
  ...
  actions: {
    ...
    async doExport({ commit, getters }) {
      try {
        if (!noticeListExporterFields || !noticeListExporterFields.length) {
          throw new Error('noticeListExporterFields is required');
        }
