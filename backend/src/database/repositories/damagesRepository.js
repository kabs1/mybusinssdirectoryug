...
class DamagesRepository extends AbstractEntityRepository {
  async create(data, options) {
    const record = {
      id: this.newId(),
      ...new Damages().cast(data),
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Damages().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Damages().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Damages().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('damages', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.damageRef) {
        query.appendIlike('damageRef', filter.damageRef);
      }

      if (filter.damageProduct) {
        query.appendId('damageProduct', filter.damageProduct);
      }

      if (filter.damageDateRange) {
        query.appendRange(
          'damageDate',
          filter.damageDateRange,
        );
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'damageRef_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendIlike('damageRef', search);
    }
  ...
  async populate(record) {
    ...
    record.damageProduct = await this.findRelation(
      'products',
      record.damageProduct,
    );

    record.damageStall =
      (await this.findRelation('stall', record.damageStall)) ||
      [];

    record.damageCausedBy =
      (await this.findRelation('user', record.damageCausedBy)) ||
      [];
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new Damages().modelName,
        entityId: id,
        action,
        values: data,
      },
...
module.exports = DamagesRepository;
