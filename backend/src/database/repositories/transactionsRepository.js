...
class TransactionsRepository extends AbstractEntityRepository {
  async create(data, options) {
    const record = {
      id: this.newId(),
      ...new Transactions().cast(data),
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Transactions().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Transactions().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Transactions().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('transactions', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.transactionRef) {
        query.appendIlike('transactionRef', filter.transactionRef);
      }

      if (filter.transactionClass) {
        query.appendEqual('transactionClass', filter.transactionClass);
      }

      if (filter.transactionType) {
        query.appendEqual('transactionType', filter.transactionType);
      }

      if (filter.transactionIdentifier) {
        query.appendIlike('transactionIdentifier', filter.transactionIdentifier);
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'transactionRef_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendIlike('transactionRef', search);
    }
  ...
  async populate(record) {
    ...
    record.transactionSession = await this.findRelation(
      'deviceSessions',
      record.transactionSession,
    );
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new Transactions().modelName,
        entityId: id,
        action,
        values: data,
      },
