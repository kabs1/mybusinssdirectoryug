...
class MemosRepository extends AbstractEntityRepository {
  async create(data, options) {
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Memos().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Memos().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Memos().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('memos', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.memosTitle) {
        query.appendIlike('memosTitle', filter.memosTitle);
      }

      if (filter.memosTo) {
        query.appendIlike('memosTo', filter.memosTo);
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'memosTitle_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendIlike('memosTitle', search);
    }
  ...
  async populate(record) {
    ...
    record.memosFrom =
      (await this.findRelation('staff', record.memosFrom)) ||
      [];

    record.memoStall =
      (await this.findRelation('stall', record.memoStall)) ||
      [];
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new Memos().modelName,
        entityId: id,
        action,
        values: data,
      },
