...
class AttendanceRepository extends AbstractEntityRepository {
  async create(data, options) {
    const record = {
      id: this.newId(),
      ...new Attendance().cast(data),
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Attendance().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Attendance().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Attendance().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('attendance', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.attendanceRef) {
        query.appendIlike('attendanceRef', filter.attendanceRef);
      }

      if (filter.attendanceStaff) {
        query.appendId('attendanceStaff', filter.attendanceStaff);
      }

      if (filter.attendanceClockInRange) {
        query.appendRange(
          'attendanceClockIn',
          filter.attendanceClockInRange,
        );
      }

      if (filter.attendanceClockOutRange) {
        query.appendRange(
          'attendanceClockOut',
          filter.attendanceClockOutRange,
        );
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'attendanceRef_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendIlike('attendanceRef', search);
    }
  ...
  async populate(record) {
    ...
    record.attendanceStaff = await this.findRelation(
      'staff',
      record.attendanceStaff,
    );
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new Attendance().modelName,
        entityId: id,
        action,
        values: data,
      },
