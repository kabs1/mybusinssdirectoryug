...
class ProductCountRepository extends AbstractEntityRepository {
  async create(data, options) {
    const record = {
      id: this.newId(),
      ...new ProductCount().cast(data),
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new ProductCount().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new ProductCount().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new ProductCount().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('productCount', id);
  ...
  async findAllAutocomplete(search, limit) {
    ...
    if (search) {
      ...
      query.appendEqual('count', search);
    }
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new ProductCount().modelName,
        entityId: id,
        action,
        values: data,
      },
...
module.exports = ProductCountRepository;
