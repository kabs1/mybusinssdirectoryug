...
class ExpenseCategoryRepository extends AbstractEntityRepository {
  async create(data, options) {
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new ExpenseCategory().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new ExpenseCategory().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new ExpenseCategory().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('expenseCategory', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.expCategoryTitle) {
        query.appendIlike('expCategoryTitle', filter.expCategoryTitle);
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'expCategoryTitle_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendIlike('expCategoryTitle', search);
    }
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new ExpenseCategory().modelName,
        entityId: id,
        action,
        values: data,
      },
