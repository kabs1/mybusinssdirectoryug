...
class BrandsRepository extends AbstractEntityRepository {
  async create(data, options) {
    const record = {
      id: this.newId(),
      ...new Brands().cast(data),
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Brands().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Brands().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Brands().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('brands', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.brandTitle) {
        query.appendIlike('brandTitle', filter.brandTitle);
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'brandTitle_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendIlike('brandTitle', search);
    }
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new Brands().modelName,
        entityId: id,
        action,
        values: data,
      },
