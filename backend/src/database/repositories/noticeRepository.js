...
class NoticeRepository extends AbstractEntityRepository {
  async create(data, options) {
    const record = {
      id: this.newId(),
      ...new Notice().cast(data),
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Notice().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Notice().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Notice().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('notice', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.noticeTitle) {
        query.appendIlike('noticeTitle', filter.noticeTitle);
      }

      if (filter.noticeType) {
        query.appendEqual('noticeType', filter.noticeType);
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'noticeTitle_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendIlike('noticeTitle', search);
    }
  ...
  async populate(record) {
    ...
    record.noticeDevice =
      (await this.findRelation('devices', record.noticeDevice)) ||
      [];

    record.noticeStall =
      (await this.findRelation('stall', record.noticeStall)) ||
      [];

    record.userIds =
      (await this.findRelation('user', record.userIds)) ||
      [];
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new Notice().modelName,
        entityId: id,
        action,
        values: data,
      },
