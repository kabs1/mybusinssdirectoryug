...
class DocumentTypesRepository extends AbstractEntityRepository {
  async create(data, options) {
    const record = {
      id: this.newId(),
      ...new DocumentTypes().cast(data),
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new DocumentTypes().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new DocumentTypes().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new DocumentTypes().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('documentTypes', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.documentTypeTitle) {
        query.appendIlike('documentTypeTitle', filter.documentTypeTitle);
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'documentTypeTitle_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendIlike('documentTypeTitle', search);
    }
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new DocumentTypes().modelName,
        entityId: id,
        action,
        values: data,
      },
