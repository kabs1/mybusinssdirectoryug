...
class TaxClassRepository extends AbstractEntityRepository {
  async create(data, options) {
    const record = {
      id: this.newId(),
      ...new TaxClass().cast(data),
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new TaxClass().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new TaxClass().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new TaxClass().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('taxClass', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.taxClassName) {
        query.appendIlike('taxClassName', filter.taxClassName);
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'taxClassName_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendIlike('taxClassName', search);
    }
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new TaxClass().modelName,
        entityId: id,
        action,
        values: data,
      },
...
module.exports = TaxClassRepository;
