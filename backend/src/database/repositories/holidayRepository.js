...
class HolidayRepository extends AbstractEntityRepository {
  async create(data, options) {
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Holiday().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Holiday().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Holiday().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('holiday', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.holidayTitle) {
        query.appendIlike('holidayTitle', filter.holidayTitle);
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'holidayTitle_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendIlike('holidayTitle', search);
    }
  ...
  async populate(record) {
    ...
    record.holidayStall =
      (await this.findRelation('stall', record.holidayStall)) ||
      [];

    record.holidayStaff =
      (await this.findRelation('staff', record.holidayStaff)) ||
      [];
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new Holiday().modelName,
        entityId: id,
        action,
        values: data,
      },
