...
class AssetsRepository extends AbstractEntityRepository {
  async create(data, options) {
    const record = {
      id: this.newId(),
      ...new Assets().cast(data),
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Assets().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Assets().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Assets().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('assets', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.assetName) {
        query.appendIlike('assetName', filter.assetName);
      }

      if (filter.assetPurchaseDateRange) {
        query.appendRange(
          'assetPurchaseDate',
          filter.assetPurchaseDateRange,
        );
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'assetName_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendIlike('assetName', search);
    }
  ...
  async populate(record) {
    ...
    record.assetsStall =
      (await this.findRelation('stall', record.assetsStall)) ||
      [];
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new Assets().modelName,
        entityId: id,
        action,
        values: data,
      },
