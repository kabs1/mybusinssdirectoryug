...
class CouponRepository extends AbstractEntityRepository {
  async create(data, options) {
    const record = {
      id: this.newId(),
      ...new Coupon().cast(data),
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Coupon().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Coupon().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Coupon().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('coupon', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.couponCode) {
        query.appendIlike('couponCode', filter.couponCode);
      }

      if (filter.couponType) {
        query.appendEqual('couponType', filter.couponType);
      }

      if (filter.couponStall) {
        query.appendId('couponStall', filter.couponStall);
      }

      if (filter.couponStatus) {
        query.appendEqual('couponStatus', filter.couponStatus);
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'couponCode_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendIlike('couponCode', search);
    }
  ...
  async populate(record) {
    ...
    record.couponStall = await this.findRelation(
      'stall',
      record.couponStall,
    );

    record.couponSalesUsed =
      (await this.findRelation('sales', record.couponSalesUsed)) ||
      [];
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new Coupon().modelName,
        entityId: id,
        action,
        values: data,
      },
