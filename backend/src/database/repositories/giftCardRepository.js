...
class GiftCardRepository extends AbstractEntityRepository {
  async create(data, options) {
    const record = {
      id: this.newId(),
      ...new GiftCard().cast(data),
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new GiftCard().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new GiftCard().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new GiftCard().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('giftCard', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.giftCardName) {
        query.appendIlike('giftCardName', filter.giftCardName);
      }

      if (filter.giftCardNoRange) {
        query.appendRange(
          'giftCardNo',
          filter.giftCardNoRange,
        );
      }

      if (filter.giftCardCustomer) {
        query.appendId('giftCardCustomer', filter.giftCardCustomer);
      }

      if (filter.giftCardStall) {
        query.appendId('giftCardStall', filter.giftCardStall);
      }

      if (filter.giftCardStatus) {
        query.appendEqual('giftCardStatus', filter.giftCardStatus);
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'giftCardNo_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendEqual('giftCardNo', search);
    }
  ...
  async populate(record) {
    ...
    record.giftCardStall = await this.findRelation(
      'stall',
      record.giftCardStall,
    );

    record.giftCardCustomer = await this.findRelation(
      'user',
      record.giftCardCustomer,
    );
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new GiftCard().modelName,
        entityId: id,
        action,
        values: data,
      },
