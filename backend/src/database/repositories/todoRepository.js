...
class TodoRepository extends AbstractEntityRepository {
  async create(data, options) {
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Todo().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Todo().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Todo().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('todo', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.todoTitle) {
        query.appendIlike('todoTitle', filter.todoTitle);
      }

      if (filter.todoStatus) {
        query.appendEqual('todoStatus', filter.todoStatus);
      }

      if (filter.todoRatingRange) {
        query.appendRange(
          'todoRating',
          filter.todoRatingRange,
        );
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'todoTitle_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendIlike('todoTitle', search);
    }
  ...
  async populate(record) {
    ...
    record.todoStaff =
      (await this.findRelation('staff', record.todoStaff)) ||
      [];

    record.todoStall =
      (await this.findRelation('stall', record.todoStall)) ||
      [];
