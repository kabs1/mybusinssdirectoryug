...
class ProductValueRepository extends AbstractEntityRepository {
  async create(data, options) {
    const record = {
      id: this.newId(),
      ...new ProductValue().cast(data),
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new ProductValue().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new ProductValue().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new ProductValue().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('productValue', id);
  ...
  async findAllAutocomplete(search, limit) {
    ...
    if (search) {
      ...
      query.appendEqual('value', search);
    }
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new ProductValue().modelName,
        entityId: id,
        action,
        values: data,
      },
...
module.exports = ProductValueRepository;
