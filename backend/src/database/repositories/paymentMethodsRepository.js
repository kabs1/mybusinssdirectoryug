...
class PaymentMethodsRepository extends AbstractEntityRepository {
  async create(data, options) {
    const record = {
      id: this.newId(),
      ...new PaymentMethods().cast(data),
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new PaymentMethods().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new PaymentMethods().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new PaymentMethods().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('paymentMethods', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.paymentMethodTitle) {
        query.appendIlike('paymentMethodTitle', filter.paymentMethodTitle);
      }

      if (filter.paymentMethodType) {
        query.appendEqual('paymentMethodType', filter.paymentMethodType);
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'paymentMethodTitle_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendIlike('paymentMethodTitle', search);
    }
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new PaymentMethods().modelName,
        entityId: id,
        action,
        values: data,
      },
