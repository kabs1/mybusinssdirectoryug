...
class CurrencyRepository extends AbstractEntityRepository {
  async create(data, options) {
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Currency().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Currency().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Currency().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('currency', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.currencyTitle) {
        query.appendIlike('currencyTitle', filter.currencyTitle);
      }

      if (filter.currencyCode) {
        query.appendIlike('currencyCode', filter.currencyCode);
      }

      if (filter.currencySign) {
        query.appendIlike('currencySign', filter.currencySign);
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'currencyTitle_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendIlike('currencyTitle', search);
    }
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new Currency().modelName,
        entityId: id,
        action,
        values: data,
      },
