...
class DevicesRepository extends AbstractEntityRepository {
  async create(data, options) {
    const record = {
      id: this.newId(),
      ...new Devices().cast(data),
    ...
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Devices().collectionName}/${record.id}`),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await AbstractEntityRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Devices().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Devices().collectionName);
  ...
  async findById(id) {
    const record = await this.findDocument('devices', id);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.deviceName) {
        query.appendIlike('deviceName', filter.deviceName);
      }

      if (filter.deviceSerial) {
        query.appendIlike('deviceSerial', filter.deviceSerial);
      }

      if (filter.deviceType) {
        query.appendEqual('deviceType', filter.deviceType);
      }

      if (filter.deviceCurrency) {
        query.appendId('deviceCurrency', filter.deviceCurrency);
      }

      if (filter.deviceStall) {
        query.appendId('deviceStall', filter.deviceStall);
      }
  ...
  async findAllAutocomplete(search, limit) {
    const query = FirebaseQuery.forAutocomplete({
      limit,
      orderBy: 'deviceSerial_ASC',
    });

    if (search) {
      query.appendId('id', search);
      query.appendIlike('deviceSerial', search);
    }
  ...
  async populate(record) {
    ...
    record.deviceCurrency = await this.findRelation(
      'currency',
      record.deviceCurrency,
    );

    record.deviceStall = await this.findRelation(
      'stall',
      record.deviceStall,
    );
  ...
  async _auditLogs(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new Devices().modelName,
        entityId: id,
        action,
        values: data,
      },
